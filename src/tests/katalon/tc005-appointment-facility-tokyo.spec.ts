...
test.describe('E2E - Successful Make Appointment Flow', () => {
    test('TC-005: Book appointment with Tokyo CURA Healthcare Center', async ({ page }) => {
        ...
        await test.step('Verify readmission is unchecked', async () => {
        ...
        await test.step('Enter visit date', async () => {
            ...
            await expect(page.locator('#txt_visit_date')).toHaveValue('25/03/2026');
        });

        // 6. Click 'Book Appointment' and verify confirmation
